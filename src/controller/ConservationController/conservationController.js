import Conservation from "../../models/ConservationModels/Conservation.js";
...
export const createConservation = async (req, res) => {
  try {
    ...
    const newDoc = await Conservation.create({
      ...formDataParsed,
...
export const getAllConservation = async (req, res) => {
  try {
    const data = await Conservation.find().sort({
      createdAt: -1,
    });
...
export const getConservationById = async (req, res) => {
  try {
    const page = await Conservation.findById(req.params.id);

    if (!page) {
      return res.status(404).json({
        message: "Conservation page not found",
      });
    }
...
export const updateConservation = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const updated = await Conservation.findByIdAndUpdate(id, updateData, {
      new: true,
      runValidators: true,
    });
...
export const deleteConservation = async (req, res) => {
  try {
    const doc = await Conservation.findById(req.params.id);
    ...
    await Conservation.findByIdAndDelete(req.params.id);
