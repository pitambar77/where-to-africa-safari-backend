import Aboutus from "../../models/AboutusModels/Aboutus.js";
...
export const getAllAboutus = async (req, res) => {
  try {
    const data = await Aboutus.find().sort({
      createdAt: -1,
    });
...
export const getAboutusById = async (req, res) => {
  try {
    const page = await Aboutus.findById(req.params.id);
...
export const updateAboutus = async (req, res) => {
  try {
    ...
    const updated = await Aboutus.findByIdAndUpdate(id, updateData, {
      new: true,
      runValidators: true,
    });
